...
const RocketCard = ({ rocket, index }: { rocket: Rocket, index: number }) => {
    ...
    const { name, flickr_images, cost_per_launch, description, diameter, height, mass, engines, first_flight, stages, first_stage, second_stage, payload_weights } = rocket
    ...
    return (
        <Card className="bg-primary-foreground my-6 grid grid-cols-1 md:grid-cols-2 rounded-none">
            <CardHeader className={`order-1 ${getOrderClasses().header}`}>
                <CardTitle className="text-2xl my-2">{name}</CardTitle>
                <CardDescription>
            ...
            <CardContent className={`order-2 ${getOrderClasses().content}`}>
                ...
                <p>First flight : <span>{first_flight}</span></p>
                <p>Coast par launch : <span>{cost_per_launch}</span>$</p>
                ...
                <p>{engines.number}&nbsp;{engines.type}(s)</p>
